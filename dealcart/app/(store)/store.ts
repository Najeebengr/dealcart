...
interface BasketState {
    ...
    removeGroupedItem: (productId: string) => void;
...
persist((set, get) => ({
    ...
    removeItem: (productId) => set((state)=>({
        ...
        removeGroupedItem: (productId) => set((state) => ({
            items: state.items.filter((item) => item.product._id !== productId),

        })),
        getTotalDiscountedPrice: () => {
            return get().items.reduce((total, item) => total + (item.product.discountedPrice ?? 0) * item.quantity, 0)
        }
    }),
    {
    name: 'basket-store',
